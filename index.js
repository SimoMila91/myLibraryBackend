...
const favoriteBook = (req, res) => {
  ...
  const query = `
        ...
        WHERE idBook = '${req.body.idBook}'
    `;
  const insertUserBook = `INSERT INTO UserBooks(idUser, idBook, favorite) VALUES(${req.body.idUser}, '${req.body.idBook}', ${req.body.favorite})`;
  db.query(query, (err, ress) => {
    if (err) {
      res.status(500).send(err);
    } else {
      if (ress.length === 0) {
        ...
        for (let i = 0; i < ress.length && !equal; i++) {
          if (ress[i].idUser == req.body.idUser) {
...
const updateType = (req, res) => {
  ...
  let query = `UPDATE UserBooks SET type = ${type} WHERE idUser = ${req.body.idUser} AND idBook = '${req.body.idBook}'`;
  db.query(query, (error, results) => {
    if (error) {
      res.status(500).send(error);
    } else {
...
const typeDefault = (req, res) => {
  let query = `UPDATE UserBooks SET type = ${null} WHERE idUser = ${req.body.idUser}  AND idBook = '${req.body.idBook}'`;
  db.query(query, (err, ress) => {
    if (err) {
      res.status(500).send(err);
    } else {
...
const updateFavorite = (req, res) => {
  let query = `UPDATE UserBooks SET favorite = ${0} WHERE idUser = ${req.body.idUser}  AND idBook = '${req.body.idBook}'`;
  db.query(query, (err, ress) => {
    if (err) {
      res.status(500).send(err);
    } else {
      res.status(200).send("Book deleted from your favorites");
    }
  });
};
...
app.put("/typeDefault", typeDefault);
app.put("/updateFavorite", updateFavorite);
app.put("/updateType", updateType);
app.put("/changePassword", changePassword);
app.post("/checkPsw", checkPassword);
app.delete("/deleteAccount", deleteAccount);
app.post("/signup", register);
app.post("/insert", checkCredentials, checkBook, insBook);
app.post("/favorite", checkCredentials, checkBook, favoriteBook);
